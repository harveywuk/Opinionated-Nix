...
- **Previous baseline**: `quattro` @ **`f0020448` (v4.0.0)**, synced August 15 2026 (326 commits from `4d93ad58`). Highlights:
  ...
  - **passwordless sudo (open question)**: `omarchy-sudo-passwordless` sets `security.sudo.wheelNeedsPassword = false`, which is blanket -- every wheel user, every command. That is broader than this port's stance elsewhere, where NOPASSWD is either narrowly scoped (`timedatectl set-timezone *`) or refused when it cannot be scoped (the `omarchy-dns` grant above). Kept blanket for now to match the menu entry's name and its placement under Setup > Security, which is a standing toggle rather than a temporary elevation. Upstream's implementation has not been compared. Revisit: if the real friction is only `nixos-rebuild`, a grant scoped to it would match the `timedatectl` precedent.
